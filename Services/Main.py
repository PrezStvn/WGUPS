...
def distance_traveled(route):
    # O(n) n in this case would be a subset of the total packages
    total_distance = 0
    for package, distance in route:
        total_distance += distance

    return total_distance

def csv_to_distance_matrix(file_path):
    # O(n^2) In this specific case I think the number of addresses in the set is 27 which amounts to a matrix with 729(n^2) cells
...
def calculate_trucks_miles(truck, address_map, distance_table):
    # O(n)
    total_distance = 0
    ...
    for package in truck:
        this_leg = float('inf')
        curr_address_index_in_distance_table = address_map[package[1]['Address']]
...
def wrapperClass(callback, address_map, distance_table):
    # my short attempt at refactoring, I quickly stopped sorry
    def inner_function(truck):
        return callback(truck, address_map, distance_table)
    return inner_function


def main():
    # big O() of any called external class or function is O(n^2)
    # no in loop call is made of any O(n^2) functions therefore the code has a complexity of O(n^2)
    # main has a runtime complexity of O(n^2)
    ...
    package_table = FileReader.read_packages(file_name)
    file_name2 = 'wgups_distance_table.csv'
    address_map = FileReader.read_distances(file_name2)
    # O(n^2 )
    ...
    truck1, truck2, truck3 = TruckLoader.load_trucks(package_table, address_map, distance_table)
    ...
    last_stop_address = address_map[truck3_last_stop[1]['Address']]
    distance_from_truck3_last_stop = distance_table[last_stop_address][0]
    return_leg = ('HUB', distance_from_truck3_last_stop)
    driver_route = wrapperClass(calculate_trucks_miles, address_map, distance_table)
    first_driver_route = driver_route(truck1)
    second_driver_route = driver_route(truck3)
    second_driver_route.append(return_leg)
    driver_2_total = distance_traveled(second_driver_route)
    print(f"driver two's total distance driven after first half of route 2, done on truck 3, completion: {driver_2_total}")
    second_driver_route = second_driver_route + driver_route(truck2)
    ...
    driver_1_total = distance_traveled(first_driver_route)
    print(f"driver one's total distance driven after route 1 completion is: {driver_1_total}")

    print(second_driver_route)
    driver_2_total = distance_traveled(second_driver_route)


    print(f"driver two's total distance after completion of route 2 with truck still in field: {driver_2_total}")
