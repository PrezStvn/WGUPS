...
def read_packages(file_name):
    # will be used to initialize the size of our hash table
    table_size = 0
    with open(file_name, 'r') as file:
        ...
        for line in file:
            table_size += 1
    #initializing our hash table to the size of packages to be delivered today(poor load factor I know)
    my_packages = HashTable.HashTable(table_size)
    with open(file_name, 'r') as file:
        # Skip irrelevant lines until headers are found
        for line in file:
            if line.startswith('Package ID'):
        ...
        for line in file:
            ...
            my_packages.map_package(package)
    return my_packages
